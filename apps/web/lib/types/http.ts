...
/**
 * Fine-grained busy substate of a RUNNING session
 * (§spec:fine-grained-busy-signal). Distinguishes a foreground turn that is
 ...
 * Monitor). Only meaningful while `state === "RUNNING"`; for every other state
 * the coarse state already tells the whole story. Delivered live over the
...
export type TaskSession = {
   ...
   * (§spec:fine-grained-busy-signal). Pushed over `session.activity_changed`;
