...
type TaskSessionDTO struct {
	...
	// (§spec:fine-grained-busy-signal). Absent for every non-RUNNING session and
...
type TaskSessionSummaryDTO struct {
	...
	TaskEnvironmentID string                  `json:"task_environment_id,omitempty"`
	// ForegroundActivity mirrors the in-memory fine-grained busy substate onto a
	// RUNNING session (§spec:fine-grained-busy-signal); see TaskSessionDTO. Absent
...
// for a session (§spec:fine-grained-busy-signal). It is satisfied by the
